...
const BUILDER_FORGE_PROMPT = `【角色锻造工序】你是角色锻造师。按顺序完成五道工序。工序一到四的思考必须写在 <thinking></thinking> 标签里——每道工序都把你的关键判断和依据写出来，不许省略：
一、规则勘察。通读下方世界书，找出所有规定「角色如何生成/存在」的条目——命名规则、种族或血统限制、数值与状态栏模板、职业或阵营体系、条目格式规定。把找到的每条规则抄进思考里并逐条遵守：这些规则是硬约束，优先级高于下面所有通用工艺。没有就写「未发现生成规则」，照常进行。
...
三、深层塑形。核心工艺。只做【本次草稿需要覆盖的部分】清单里列出的小节——清单没有的连提都不要提；清单里有的就是用户要的，不做适不适合的自判。补肉是你的职责，不是用户的：用户描述的详略就是TA意愿的信号，描述里没有的具体事实（身高、场景、习惯、关系画面），选与描述和世界观一致的合理值填进骨架——发明的事实会以草稿形式交给用户审改，不要拿笼统含糊来回避填空。每个勾选小节按下面对应的 ■ 工艺块做：先在思考里完成该块要求的判断，再按骨架落稿。
...
四、自检。只在思考里进行——清单本身、逐项确认、改稿痕迹都不出现在成稿里。快速过一遍，发现问题先回去改：①每节至少一条因果（因为X所以Y）？删掉全部形容词还站得住？读它的AI能推导没写到的情境？在讲「为什么」而不只是「是什么」？没有让AI背诵的固定台词与口癖？②一物一家：同一个比喻、同一个命题只住一个小节？规律（调色盘/关系）与实例（语料/画面）没有措辞趋同？③面孔的能量声明与弱点缺陷不打架？每种场景只有一个主人面？④点睛比喻没有锁死防误读保护的暧昧、没有焊死成长与退行的门？⑤思考里做出的每样东西都落进了成稿？⑥文风禁形逐条扫（见工序五）？
...
function soDeleteConfirmMessage(kind, name) {
    if (kind === 'preset') return `删除连接预设「${name || ''}」？此操作无法撤销。`;
            ...
            <details class="so-set-group" open>
                ...
                <div class="so-set-body">
                    ...
                    <div id="so-direct-fields">
                        ...
                        <label class="so-field"><span>连接预设</span>
                            <div class="so-conn-preset-row">
                                <select id="so-conn-preset-select"><option value="">（选择预设加载）</option></select>
                                <div class="so-iconbtn" id="so-conn-preset-load" title="加载选中的预设"><i class="fa-solid fa-download"></i></div>
                                <div class="so-iconbtn" id="so-conn-preset-del" title="删除选中的预设"><i class="fa-solid fa-trash"></i></div>
                            </div>
                        </label>
                        <div class="so-conn-preset-row">
                            <input id="so-conn-preset-name" type="text" placeholder="预设名称（存当前端点 / 密钥 / 模型）">
                            <div class="so-iconbtn" id="so-conn-preset-save" title="把当前端点 / 密钥 / 模型存成预设"><i class="fa-solid fa-floppy-disk"></i></div>
                        </div>
                    </div>
...
function bindControls() {
    ...
    win.querySelector('#so-conn-preset-save').addEventListener('click', () => {
        ...
        if (!name) { if (window.toastr) window.toastr.info('先给预设起个名字', '故事神谕'); return; }
